...
class Keywords
{
    ...
    {
        $this->extractKeywordUpdate();
        $this->message = 'Keyword wurde aktualisiert!';

        if($this->postArray['id'] > 0 && $this->postArray['name'] != '') {
            $this->db->where('keywordID', $this->postArray['id']);

            $data = [
                'keywordName'    => $this->postArray['name'],
                'keywordTraffic' => $this->postArray['sv']
            ];

            if(!$this->db->update('keywords', $data)) {
                $this->error   = 1;
                $this->message = 'Keyword wurde NICHT aktualisiert!';
            }
        }
    ...
    private function extractKeywordUpdate()
    {
        $kwDataString = $this->request->getParsedBody()['update'];
        $kwDataString = strtolower($kwDataString);
        $temp         = explode('&kwname=', $kwDataString);
        $tempID       = explode("=", $temp[0]);
        $tempSV       = explode("&kwsv=", $temp[1]);

        $this->postArray['id']   = intval($tempID[1]);
        $this->postArray['name'] = urldecode($tempSV[0]);
        $this->postArray['sv']   = intval($tempSV[1]);
...
//                }
    ...
    private function extractKeywords()
    {
        ...
        $temp                                           = explode('=', $temp);
        $this->
                currentProjectsParentID = $temp[1];
