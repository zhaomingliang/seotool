...
class Keywords
{
    ...
    private function markBestCompetitor($rankingValue)
    {
        ...
        $cssCounter         = 0;
        $competitiorCount   = count($rankingValue) - 3; //first three are id, name and traffic
    ...
    public function generateRankingTable()
    {
        ...
        foreach ($this->modelData['rankingData'] as $rankingKey => $rankingValue) {
            ...
            $html[] = '<td class="tblBtn"><i data-toggle="modal" data-target="#keywordModal" data-kid="' . $rankingValue['keywordID'] . '" data-name="' . $rankingValue['keywordName'] . '" data-sv="' . $rankingValue['keywordTraffic'] . '" class="fa fa-edit keywordEdit"></i></td>';
