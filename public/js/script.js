$( document ).ready( function() {
    ...
    ajaxRequestHandler( "#backlinkAdd_submit", "#backlinkAdd", "/ajax/backlinks/add/", "add", "#backlinkAlert", 1 );

    ajaxRequestHandler( "#backlinkUpdate_submit", "#backlinkUpdate", "/ajax/backlinks/update/", "update", "#backlinkAlert" );
    ...
    $( '#keywordModal' ).on( 'show.bs.modal', function( event ) {
        ...
        var sv = button.data( 'sv' );
        var modal = $( this );
        modal.find( '#modal-kw' ).text( keyword );
        modal.find( 'input[name="kwName"]' ).val( keyword );
        modal.find( 'input[name="kwSV"]' ).val( sv );
        modal.find( 'input[name="kid"]' ).val( kwid );
    } );
